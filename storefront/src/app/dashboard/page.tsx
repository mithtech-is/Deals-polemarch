...
export default function DashboardPage() {
    ...
    const fulfilledOrders = orders.filter((order: any) => order.fulfillment_status === "fulfilled");
    ...
    const kycConfig = {
        ...
        approved: {
            ...
            icon: Shield,
            title: "Approved",
            desc: "Your KYC has been accepted. You can now access all investment features.",
            btnText: "View KYC Details",
            pulse: false
        },
        verified: {
            color: "text-green-600",
        ...
        rejected: {
            ...
            icon: Shield,
            title: "Rejected",
            desc: user.metadata?.kyc_rejection_reason || "Your KYC was rejected. Please update your details and submit again.",
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between py-3 border-b border-slate-50">
                                        ...
                                        <span className={`text-sm font-bold flex items-center gap-1 ${isKycApproved ? "text-green-600" : isKycRejected ? "text-red-600" : "text-orange-600"}`}>
                                            <div className={`h-1.5 w-1.5 rounded-full ${isKycApproved ? "bg-green-600" : isKycRejected ? "bg-red-600" : "bg-orange-600"} ${isKycApproved || isKycRejected ? "" : "animate-pulse"}`} />
                                            {isKycApproved ? "Approved" : isKycRejected ? "Rejected" : "Pending Verification"}
                                        </span>
                                    </div>
                                    <div className="flex justify-between py-3 border-b border-slate-50">
                        ...
                        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden group">
                            ...
                            <div className="relative z-10 h-full flex flex-col">
                                ...
                                <Link
                                    href="/dashboard/kyc"
                                    className={`w-full py-4 rounded-2xl bg-primary text-white font-bold hover:shadow-lg transition-all flex items-center justify-center gap-2 mt-6 ${isKycApproved ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}
                                >
                                    {kycConfig.btnText}
                        ...
                        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden group text-foreground">
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                ...
                                <div className="text-4xl font-extrabold mb-1 tracking-tight text-primary">Rs. {totalPortfolioValue.toLocaleString("en-IN")}</div>
                                ...
                                <div className="space-y-3 mt-auto">
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <span className="font-bold">Rs. {totalMedusaOrdersValue.toLocaleString("en-IN")}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-slate-500">Manual Tracked</span>
                                        <span className="font-bold">Rs. {totalManualValue.toLocaleString("en-IN")}</span>
                                    </div>
                                    <button
                            ...
                            <div className="bg-white rounded-[40px] p-10 max-w-lg w-full shadow-2xl animate-in zoom-in-95 duration-200">
                                ...
                                <form onSubmit={handleAddManualInvestment} className="space-y-6">
                                    <div>
                                        ...
                                        <input required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-primary/20 font-medium" placeholder="e.g. Swiggy, Ola" value={manualFormData.companyName} onChange={(e) => setManualFormData({ ...manualFormData, companyName: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className="text-sm font-bold text-slate-600 block mb-2">Amount Invested (Rs.)</label>
                                        <input required type="number" className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-primary/20 font-medium" placeholder="50000" value={manualFormData.amount} onChange={(e) => setManualFormData({ ...manualFormData, amount: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className="text-sm font-bold text-slate-600 block mb-2">Platform / Source</label>
                                        <input className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-primary/20 font-medium" placeholder="e.g. Altius, Precise" value={manualFormData.platform} onChange={(e) => setManualFormData({ ...manualFormData, platform: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className="text-sm font-bold text-slate-600 block mb-2">ISIN</label>
                                        <input required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-primary/20 font-medium" placeholder="e.g. INE721I01024" value={manualFormData.isin} onChange={(e) => setManualFormData({ ...manualFormData, isin: e.target.value })} />
                                    </div>
                                    <div className="flex gap-4 pt-4">
                                        <button type="button" onClick={() => setIsAddingManual(false)} className="flex-1 py-4 rounded-2xl bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 transition-all">
                                            Cancel
                                        </button>
                                        <button type="submit" disabled={isSubmittingManual} className="flex-1 py-4 rounded-2xl bg-primary text-white font-bold hover:shadow-lg transition-all flex items-center justify-center gap-2">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm p-10">
                            ...
                            {manualInvestments.length > 0 ? (
                                    ...
                                    {manualInvestments.map((inv: any) => (
                                        <div key={inv.id} className="flex items-center justify-between p-6 rounded-3xl bg-slate-50/50 border border-slate-100 group">
                                            ...
                                            <div className="flex items-center gap-6">
                                                <p className="font-bold text-primary">Rs. {parseFloat(inv.amount).toLocaleString("en-IN")}</p>
                                                <button onClick={() => handleDeleteManualInvestment(inv.id)} className="p-2 text-slate-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                                ...
                                <div className="py-20 text-center">
                                    ...
                                    <button onClick={() => setIsAddingManual(true)} className="mt-4 text-primary font-bold hover:underline">
                        ...
                        <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm p-10 overflow-hidden relative">
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <button onClick={fetchOrders} className="p-2 hover:bg-slate-50 rounded-full transition-colors group" title="Refresh Transactions">
                            ...
                            {ordersLoading ? (
                                ...
                                <div className="space-y-4">
                                        ...
                                        return (
                                            <div key={order.id} className="p-6 rounded-3xl bg-slate-50/50 border border-slate-100 flex items-center justify-between group hover:bg-slate-50 transition-all">
                                                ...
                                                <div className="text-right">
                                                    <p className="font-bold text-slate-900 mb-2">Rs. {order.total.toLocaleString("en-IN")}</p>
                                    ...
                                    {orders.length > visibleCount && (
                                        <button onClick={() => setVisibleCount((prev) => prev + 5)} className="w-full py-4 bg-slate-50 text-slate-600 font-bold rounded-2xl border border-dashed border-slate-200 hover:bg-slate-100 transition-all text-xs">
